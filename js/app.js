...
(function () {
    ...
    // ===== STATE =====
    let dataC1 = JSON.parse(localStorage.getItem('qc_dataC1')) || [];
    let barChart = null;
    let pieChart = null;
    ...
    function updateTPSDropdown() {
        // Disable TPS options that already have data
    ...
    function bindEvents() {
        ...
        el.btnResetForm.addEventListener('click', () => { el.formC1.reset(); el.suaraSah.value = 0; el.totalSahTidakSah.value = 0; showNotification('Form direset', 'success'); });
        el.btnResetData.addEventListener('click', handleResetData);

        // Auto-calculate Suara Sah + Tidak Sah
        el.suaraTidakSah.addEventListener('input', hitungTotalSuara);

        // Listen to kandidat inputs for auto-sum suara sah
        for (let i = 0; i < 5; i++) {
    ...
    function hitungSuaraSah() {
        ...
        for (let i = 0; i < 5; i++) {
            const input = document.getElementById(`suaraCalon_${i}`);
            totalSah += parseInt(input.value) || 0;
        }
    ...
    function navigateToModule(name) {
        ...
        if (name === 'dashboard') { el.moduleDashboard.classList.add('active'); updateDashboard(); }
    ...
    function handleFormSubmit(e) {
        ...
        konfirmasi += `\nSuara Sah: ${suaraSah}`;
        konfirmasi += `\nTotal Sah + Tidak Sah: ${suaraSah + suaraTidakSah}`;
        konfirmasi += `\nSurat Tidak Terpakai: ${suratTidakTerpakai}`;

        if (!confirm(konfirmasi)) return;

        // Block duplicate TPS
        const existIdx = dataC1.findIndex(d => d.nomorTPS === nomorTPS);
        if (existIdx !== -1) {
            showNotification(`TPS ${nomorTPS} sudah diinput! Tidak bisa input ulang.`, 'error');
            return;
        }
    ...
    // ===== RENDER DETAIL TABLE =====
    let editingTPS = null; // Track which TPS is being edited
    ...
    function renderDetailTable() {
        ...
        dataC1.forEach((d, idx) => {
            ...
            if (isEditing) {
                ...
                html += `<button class="btn btn-danger btn-sm" onclick="window.hapusTPS(${d.nomorTPS},${d.rw})"><i class="fas fa-trash"></i></button>`;
            }
            html += `</td>`;
            html += '</tr>';
        });
    ...
    window.simpanEdit = function (idx) {
        ...
        if (!confirm(`Simpan perubahan data TPS ${d.nomorTPS} RW ${d.rw}?`)) return;

        dataC1[idx].suara = newSuara;
        dataC1[idx].totalSuara = newTotal;
        dataC1[idx].timestamp = new Date().toISOString();

        editingTPS = null;
        saveData();
        updateDashboard();
        showNotification(`Data TPS ${d.nomorTPS} berhasil diperbarui!`, 'success');
    };
    ...
    function updateDashboard() {
        ...
        // TPS badges
        if (dataC1.length > 0) {
            const badges = dataC1.map(d => `<span class="tps-badge">TPS ${d.nomorTPS}</span>`).join('');
            el.tpsBadges.innerHTML = badges;
        } else {
    ...
    // ===== UTILITY =====
    function saveData() { localStorage.setItem('qc_dataC1', JSON.stringify(dataC1)); }
    ...
    window.hapusTPS = function (tps, rw) {
        if (!confirm(`Hapus data TPS ${tps} RW ${rw}?`)) return;
        dataC1 = dataC1.filter(d => !(d.nomorTPS === tps && d.rw === rw));
        saveData();
        updateDashboard();
        updateTPSDropdown();
        showNotification(`Data TPS ${tps} RW ${rw} dihapus`, 'warning');
    };
